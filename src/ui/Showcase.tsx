...
export function Showcase({ session, panel, presented, cursor, playerTotal, infoOn, lastAdvanceMs, intervalMs, animate, live, marker, width, height, commits, full, progress, filesSort, tasksHideDone, paletteOpen, paletteQuery, paletteGhost, reveal, focusLocked }: Props) {
  ...
  return (
    <box style={{ flexGrow: 1, flexDirection: "column", backgroundColor: TRANSPARENT }}>
      ...
      >
        {panel === "lens" && <Lens presented={presented} cursor={cursor} total={playerTotal} animate={animate} live={live} lastAdvanceMs={lastAdvanceMs} intervalMs={intervalMs} status={session.status} infoOn={infoOn} tokens={agg.tokens} ctxPools={agg.ctxPools} width={width - 4} height={bodyHeight} />}
